 ...
 */
export function initStarknet(privateKey) {
 ...
 */
export async function getSageBalance(client, address) {
    ...
    return uint256ToBigInt(result);
}
/**
 * Claim from faucet
 */
export async function claimFromFaucet(client) {
    if (!client.account) {
 ...
 */
export async function registerValidator(client, stakeAmount, commissionBps = 500, attestationHash = "0") {
    if (!client.account) {
        throw new Error("Wallet not connected");
    }
 ...
 */
export async function addStake(client, amount) {
    if (!client.account) {
        throw new Error("Wallet not connected");
    }
 ...
 */
export async function removeStake(client, amount) {
    if (!client.account) {
        throw new Error("Wallet not connected");
    }
    const { low, high } = bigIntToUint256(amount);
 ...
 * Get validator info
 */
export async function getValidatorInfo(client, address) {
    const contract = new Contract(VALIDATOR_REGISTRY_ABI, client.addresses.VALIDATOR_REGISTRY, client.provider);
    try {
 ...
 */
export async function isActiveValidator(client, address) {
    const contract = new Contract(VALIDATOR_REGISTRY_ABI, client.addresses.VALIDATOR_REGISTRY, client.provider);
    try {
 ...
 * Get network stats
 */
export async function getNetworkStats(client) {
    const contract = new Contract(VALIDATOR_REGISTRY_ABI, client.addresses.VALIDATOR_REGISTRY, client.provider);
    try {
 ...
 */
export async function waitForTransaction(client, txHash) {
    try {
...
function uint256ToBigInt(value) {
