...
const DENOMINATIONS = [10, 100, 1000];
export async function privacyCommand(subcommand, args, options) {
    switch (subcommand) {
        case "deposit":
...
async function handleDeposit(amountArg, options) {
    console.log("");
    console.log(chalk.blue("═══════════════════════════════════════════════════════════════"));
    ...
    try {
        ...
        // Select denomination
        let denomination;
        if (amountArg && DENOMINATIONS.includes(parseInt(amountArg))) {
            denomination = parseInt(amountArg);
        }
        else {
            const answer = await inquirer.prompt([
                {
        ...
        const txHash = await executeDeposit(client.account, addresses, depositAmount, commitment);
...
async function handleWithdraw(indexArg, options) {
    console.log("");
    console.log(chalk.blue("═══════════════════════════════════════════════════════════════"));
    ...
    try {
        ...
        const proof = await requestWithdrawalProof(config, selectedNote, config.wallet.address, compliance.level);
        spinner.text = "Submitting withdrawal transaction...";
        // Execute withdrawal
        const txHash = await executeWithdrawal(client.account, addresses, proof, config.wallet.address);
...
async function handleNotes(options) {
    console.log("");
    console.log(chalk.blue("═══════════════════════════════════════════════════════════════"));
...
async function handleSets(options) {
    console.log("");
    console.log(chalk.blue("═══════════════════════════════════════════════════════════════"));
...
async function executeDeposit(account, addresses, amount, commitment) {
...
async function executeWithdrawal(account, addresses, proof, recipient) {
    const result = await account.execute({
        contractAddress: addresses.PRIVACY_POOLS || "0x0",
...
function bigIntToUint256(value) {
