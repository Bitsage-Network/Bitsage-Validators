...
export async function detectGPU() {
    ...
    try {
        ...
        if (graphics.controllers && graphics.controllers.length > 0) {
            ...
            else if (gpu.vendor?.toLowerCase().includes("apple") ||
...
function estimateVRAM(model) {
    ...
    if (modelLower.includes("3090"))
        return 24;
    if (modelLower.includes("3080 ti"))
        return 12;
    if (modelLower.includes("3080"))
        return 10;
    if (modelLower.includes("3070"))
        return 8;
    if (modelLower.includes("3060"))
        return 12;
    // NVIDIA mid-range
    if (modelLower.includes("2080 ti"))
        return 11;
    if (modelLower.includes("2080"))
        return 8;
    if (modelLower.includes("2070"))
        return 8;
    if (modelLower.includes("2060"))
        return 6;
    if (modelLower.includes("1080 ti"))
        return 11;
    if (modelLower.includes("1080"))
        return 8;
    if (modelLower.includes("1070"))
        return 8;
    // AMD
    if (modelLower.includes("rx 7900"))
        return 24;
    if (modelLower.includes("rx 7800"))
        return 16;
    if (modelLower.includes("rx 6900"))
        return 16;
    if (modelLower.includes("rx 6800"))
        return 16;
    if (modelLower.includes("rx 6700"))
        return 12;
    // Apple Silicon
    if (modelLower.includes("m3 max"))
        return 40;
    if (modelLower.includes("m3 pro"))
        return 18;
    if (modelLower.includes("m2 max"))
        return 32;
    if (modelLower.includes("m2 pro"))
        return 16;
    if (modelLower.includes("m1 max"))
        return 32;
    if (modelLower.includes("m1 pro"))
        return 16;
    if (modelLower.includes("apple m"))
        return 8;
    return 4;
}
/**
 ...
 * GPU command handler
 */
export async function gpuCommand(subcommand, options) {
    switch (subcommand) {
        case "detect":
...
async function handleSet(options) {
    ...
    const type = options.type || config.gpu.type;
    // Validate
    if (tier < 0 || tier > 4) {
