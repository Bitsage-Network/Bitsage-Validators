...
import { getContractAddresses, useSageBalance } from "@/lib/contracts";
...
export function ObelyskWalletProvider({ children }: { children: ReactNode }) {
  ...
  const rollover = useCallback(async () => {
    ...
    try {
      ...
      if (claimRewards) {
        ...
        // Fallback: privacy client direct call
        if (!privacyClient) {
          throw new Error("Privacy client not initialized");
        }

        // Privacy client rollover is not yet implemented in the SDK.
        // Throwing an error prevents silently faking a successful rollover
        // which would leave on-chain state unchanged.
        throw new Error(
          "Rollover via privacy client is not yet supported. " +
          "Use the SDK's claimRewards hook or wait for privacy client rollover support."
        );
      }
    } catch (error) {
  ...
  }, [balance.pending, privacyClient, claimRewards, refetchSageBalance]);
  ...
  const ragequit = useCallback(async () => {
    ...
    try {
      // Ragequit withdraws the entire private balance back to the public ERC-20.
      // The privacy client SDK does not yet expose a ragequit method.
      // When it does, replace this block with:
      //   const result = await privacyClient.ragequit();
      ...
      throw new Error(
        ...
        "Use usePrivacyPool().initiateRagequit() or buildPrivacyPoolRagequitCall " +
  ...
  }, [decryptedPrivateBalance, privacyClient]);
  ...
  const sendPrivate = useCallback(async (to: string, amount: string) => {
    ...
    try {
      ...
      if (executePrivateTransfer && typeof (executePrivateTransfer as unknown) === 'function') {
        ...
        // Fallback: privacy client direct transfer
        if (!privacyClient) {
          throw new Error("Privacy client not initialized");
        }

        // The privacy client's direct transfer method is not yet implemented.
        // Throwing prevents silently faking a successful transfer that never
        // touches the chain — which would desync local and on-chain state.
        throw new Error(
