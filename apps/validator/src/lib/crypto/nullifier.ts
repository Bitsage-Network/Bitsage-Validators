...
// View tag for efficient scanning (Fuzzy Message Detection)
//
// FMD Precision State:
//   - Fixed 16-bit tag derived from Poseidon(sharedSecret, outputIndex)
//   - False positive rate: ~1/65536 per output (~0.0015%)
//   - This is a static precision — no server-side detection key rotation.
//   - Sufficient for testnet scanning loads. For mainnet with high tx volume,
//     consider upgrading to variable-precision FMD (e.g., Penumbra-style
//     FMD with configurable false-positive rates via detection keys).
//   - The view tag allows light clients to filter irrelevant outputs without
//     attempting full decryption, reducing scan cost from O(n*decrypt) to O(n*hash).
//
