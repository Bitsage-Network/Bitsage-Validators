 ...
 *
 * Security Model:
 *   - KEK (Key Encryption Key) is derived from a deterministic wallet signature
 *     over a domain-specific message using HKDF-SHA256.
 *   - The ElGamal private key is encrypted with AES-GCM-256 using the KEK.
 *   - Encrypted key material is stored in IndexedDB (origin-scoped).
 *   - KEK is never persisted — it is re-derived on each session from a fresh
 *     wallet signature, so key access requires wallet unlock (hardware/software).
 *   - Export/import uses PBKDF2 with 100,000 iterations for password-based
 *     encryption, suitable for offline backup transport.
 *
 * Threat surface:
 *   - XSS can read IndexedDB but cannot decrypt without the wallet signature.
 *   - Compromised wallet (private key leak) allows KEK re-derivation.
 *   - The signature message is domain-bound to prevent cross-app replay.
 *
 * Flow:
