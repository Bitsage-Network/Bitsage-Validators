...
export function validateEnv(): { valid: boolean; errors: string[] } {
  ...
  // Mainnet-specific hardened validation
  if (network === 'mainnet') {
    // RPC URL must not be a testnet/demo URL
    const rpcUrl = getEnv('NEXT_PUBLIC_RPC_URL') || '';
    if (rpcUrl.includes('sepolia') || rpcUrl.includes('testnet') || rpcUrl.includes('demo')) {
      errors.push('NEXT_PUBLIC_RPC_URL appears to be a testnet URL. Mainnet requires a mainnet RPC endpoint.');
    }

    // All contract addresses must be valid (non-zero, non-placeholder)
    for (const envVar of CONTRACT_ENV_VARS) {
      const value = getEnv(envVar as keyof typeof ENV_VALUES);
      if (value && value.length < 10) {
        errors.push(`${envVar} appears to be a placeholder. Mainnet requires fully deployed contract addresses.`);
      }
    }
...
export function getEnvConfig(): EnvConfig {
  ...
  return {
    network,
    rpcUrl: getEnv('NEXT_PUBLIC_RPC_URL') || defaultRpcUrl,
