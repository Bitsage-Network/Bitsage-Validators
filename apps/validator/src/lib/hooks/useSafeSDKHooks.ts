 ...
 */
export function useSafeValidatorStatus(): SafeHookResult<Record<string, unknown>> {
  const { isConnected } = useAccount();
  const [result, setResult] = useState<SafeHookResult<Record<string, unknown>>>({
    data: null,
    isLoading: false,
    error: null,
  });
 ...
 */
export function useSafeNetworkStatsStream(): { stats: Record<string, unknown> | null; isSubscribed: boolean } {
  const { isConnected } = useAccount();
