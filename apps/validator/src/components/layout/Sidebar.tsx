...
const networkName = process.env.NEXT_PUBLIC_STARKNET_NETWORK || "sepolia";
const networkLabel = networkName === "mainnet" ? "Starknet Mainnet" : networkName === "devnet" ? "Starknet Devnet" : "{networkLabel}";
      ...
      <div className="h-14 flex items-center justify-between px-4 border-b border-surface-border">
        <Link href="/dashboard" className="flex items-center gap-3">
          <div className="w-10 h-10 flex items-center justify-center">
            <LogoIcon className="text-emerald-400" size={36} />
          </div>
          <AnimatePresence>
            {!collapsed && (
              <motion.div
              ...
              >
                ...
                <span className="block text-xs text-emerald-400">Validator Network</span>
              </motion.div>
            )}
          </AnimatePresence>
        </Link>
      ...
      <AnimatePresence>
        {!collapsed && (
          <motion.div
            ...
            className="mx-3 mt-3 p-3 rounded-xl bg-emerald-600/10 border border-emerald-500/20"
          >
        ...
        <AnimatePresence>
          {!collapsed && (
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="mt-6 p-4 rounded-xl bg-gradient-to-br from-emerald-600/20 to-violet-500/20 border border-emerald-500/20"
            >
      ...
      {collapsed && (
        <div className="py-4 px-3">
          <div className="flex justify-center">
            <div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" title="{networkLabel}" />
          </div>
        </div>
      )}
    </motion.aside>
