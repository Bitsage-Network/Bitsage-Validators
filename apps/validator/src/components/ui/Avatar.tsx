...
const badgeConfig: Record<BadgeType, { bg: string; icon: typeof Check }> = {
  ...
  validator: { bg: "bg-emerald-500", icon: Zap },
  prover: { bg: "bg-purple-500", icon: Zap },
};
...
export function ValidatorAvatar({
  ...
  const getReputationColor = (rep: number) => {
    ...
    if (rep >= 70) return "stroke-emerald-500";
  ...
  return (
    <div className={cn("relative inline-flex", className)}>
      ...
      <span
        className={cn(
          "absolute rounded-full flex items-center justify-center bg-emerald-500",
          config.badge,
          config.badgeOffset
        )}
      >
