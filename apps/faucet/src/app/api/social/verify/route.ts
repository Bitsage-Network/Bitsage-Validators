...
async function recordSocialBonus(
  wallet: string,
  taskId: string,
  platform: string,
  taskType: string,
  socialAccount?: string,
): Promise<{ success: boolean; reward_amount_formatted?: string; transaction_hash?: string; error?: string }> {
  try {
    const res = await fetch(`${COORDINATOR_URL}/api/faucet/social-bonus`, {
      method: 'POST',
    ...
    if (res.status === 409) {
      // Already completed — still counts as verified
      return { success: true, error: 'already_completed' };
    }

    if (!res.ok) {
      console.error('Coordinator social-bonus error:', data);
      return { success: false, error: data.error || 'Coordinator error' };
    }

    return {
      success: true,
      reward_amount_formatted: data.reward_amount_formatted,
      transaction_hash: data.transaction_hash,
    };
  } catch (error) {
    console.error('Failed to call coordinator social-bonus:', error);
    // Don't fail the verification — just log the error
    return { success: false, error: 'Coordinator unavailable' };
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (taskType === 'github_star' || taskType === 'github_follow') {
      ...
      if (verified) {
        const bonus = await recordSocialBonus(
          wallet,
          taskId,
          'github',
          taskType,
        ...
        return NextResponse.json({
          ...
          reward_amount_formatted: bonus.reward_amount_formatted,
          transaction_hash: bonus.transaction_hash,
        });
      } else {
        return NextResponse.json({
          success: true,
    ...
    if (taskType === 'discord_join') {
      ...
      if (!BITSAGE_DISCORD_GUILD_ID) {
        // If guild ID not configured, accept connection as proof
        const bonus = await recordSocialBonus(
          wallet,
          taskId,
          'discord',
